...
public class BigLucky extends Weapon.Enchantment {
	...
	public int proc( Weapon weapon, Char attacker, Char defender, int damage ) {
		...
		// lvl 1 ~ 12%
		// lvl 2 ~ 14%
		float procChance = (level+8f)/(level+80f) * procChanceMultiplier(attacker);
		if (Random.Float() < procChance){
			...
			Buff.affect(defender, LuckProc.class).ringLevel = -15 + Math.round(10*powerMulti);
		} else {
